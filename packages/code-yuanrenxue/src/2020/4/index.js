...
const match = async page => {
  const url = `${domain}/api/match/4?${formatParams({ page })}`
...
const formattedRes = res => {
  const { key, value, info } = res

  const j_key = md5(btoa(key + value).replace(/=/g, ''))

  const tds = info.match(/<td>(.*?)<\/td>/g)
  const nums = tds.map(item => {
    const l_imgs = item.match(/<img[^>]*>/g)
    const showImgs = l_imgs.filter(item => !item.includes(j_key))
    const match = showImgs.map((item, index) => {
      const match = item.match(/<img[^>]*src="([^"]*)"[^>]*style="[^"]*left:([^"]*)"[^>]*>/)
      return {
        num: mapping[match[1]],
        left: Math.round(parseInt(match[2]) / 11) + index
      }
    })
    return match.sort((a, b) => a.left - b.left).map(item => item.num)
  })

  return nums.map(item => +item.join(''))
}

const addSum = data => data.reduce((pre, cur) => ((pre += cur), pre), 0)

let sum = 0

for (let i = 0; i < 5; i++) {
  const res = await match(i + 1)
  const data = formattedRes(res)
  sum += addSum(data)
  await delay()
}
